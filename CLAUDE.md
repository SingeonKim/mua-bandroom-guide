...
- **배포:** `main`에 push하면 `.github/workflows/deploy.yml`가 자동 빌드·배포한다. 별도 배포 명령 없음. 배포 URL: `https://SingeonKim.github.io/bandroom-guide/` (GitHub 레포명·Pages URL은 `bandroom-guide`로 변경됨 — 단 로컬 작업 폴더 경로는 여전히 `mua-bandroom-guide`). 워크플로우 확인: `gh run watch <id> --exit-status`.
